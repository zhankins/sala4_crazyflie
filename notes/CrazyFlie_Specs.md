# CrazyFlie Battery Specifications
https://store.bitcraze.io/collections/accessories/products/350mah-lipo-battery

[Forum on Battery Selection for the crazyflie](https://forum.bitcraze.io/viewtopic.php?f=6&t=392)

Dimensions:
General specification
---
Operating Temperature: 0~45°C (Charging) / 0~60°C (Discharging)


Electrical specification
---
Capacity: 350mAh
* Nominal voltage: 3.7 V (1 Cell)
* Max discharge: 15C continuous / 30C burst
* Charge: 1-3C recommended, 5C Max
* Connector: Molex 51005-2P (Pin 1 positive)

Mechanical specifications
---

Weight: 9.1g
Size (WxHxD): 20x8x33mm (not including cable)
Cable length: 35mm (including connector)
Wire gauge: 22 AWG

Package Contents
---
1 x 350mAh LiPo battery
