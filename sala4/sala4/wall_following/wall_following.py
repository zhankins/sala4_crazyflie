...
class WallFollowing:
    ...
    def command_align_corner(
        self, reference_rate, side_range, wanted_distance_from_corner
    ):
        """
    ...
    def wall_follower(
        ...
        if self.state == self.StateWallFollowing.FORWARD:
            if (
                front_range
                < self.reference_distance_from_wall + self.ranger_value_buffer
            ):
                self.state = self.state_transition(
                    self.StateWallFollowing.TURN_TO_FIND_WALL
                )
            ...
            #   can detect the wall
            side_range_check = side_range < (
                self.reference_distance_from_wall / math.cos(math.pi / 4)
                + self.ranger_value_buffer
            )
            front_range_check = front_range < (
                self.reference_distance_from_wall / math.cos(math.pi / 4)
                + self.ranger_value_buffer
            )
            if side_range_check and front_range_check:
                ...
                self.wall_angle = self.wall_following_direction_value * (
                    math.pi / 2
                    - math.atan(front_range / side_range)
                    + self.angle_value_buffer
                )
                self.state = self.state_transition(
                    self.StateWallFollowing.TURN_TO_ALIGN_TO_WALL
                )
            ...
            if (
                ...
                < self.reference_distance_from_wall + self.ranger_value_buffer
                and front_range
                > self.reference_distance_from_wall + self.range_threshold_lost
            ):
                self.around_corner_back_track = False
                self.prev_heading = current_heading
            ...
            if align_wall_check:
                self.state = self.state_transition(
                    self.StateWallFollowing.FORWARD_ALONG_WALL
                )
            ...
            if (
                ...
                self.prev_heading = current_heading
                self.state = self.state_transition(
                    self.StateWallFollowing.ROTATE_IN_CORNER
                )
            ...
            if side_range <= self.reference_distance_from_wall:
                self.state = self.state_transition(
                    self.StateWallFollowing.ROTATE_AROUND_WALL
                )
        else:
        ...
        if self.state == self.StateWallFollowing.FORWARD:
            ...
            (
                command_velocity_x_temp,
                command_velocity_y_temp,
            ...
            if (
                self.time_now - self.state_start_time
            ...
            if (
                ...
                if (
                    self.wrap_to_pi(math.fabs(current_heading - self.prev_heading))
                    > self.in_corner_angle
                ):
                ...
                if self.around_corner_back_track:
                    ...
                    command_velocity_y_temp, command_angle_rate_temp = (
                        self.command_turn(-1 * self.max_turn_rate)
                    )
                    command_velocity_x_temp = 0.0
                else:
                    command_velocity_y_temp, command_angle_rate_temp = (
